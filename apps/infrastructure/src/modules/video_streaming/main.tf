...
# Create a new Route53 zone if route53_zone_id is not provided but custom_domain_name is
resource "aws_route53_zone" "main" {
  count = var.custom_domain_name != null && var.route53_zone_id == null ? 1 : 0
  name  = var.custom_domain_name
}

# Use existing zone if provided, otherwise use the newly created one
...
# Create an apex DNS record for the custom domain pointing to CloudFront

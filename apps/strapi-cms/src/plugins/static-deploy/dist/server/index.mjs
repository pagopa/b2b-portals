...
const register = ({ strapi: strapi2 }) => {
  strapi2.documents.use(async (context, next) => {
    ...
    const triggerActions = isDraftAndPublish ? ["delete", "publish", "unpublish"] : ["delete", "update", "create"];
    const publishedDocumentToDelete = context.action === "delete" && isDraftAndPublish ? await strapi2.documents(context.contentType.uid).findOne({ documentId: context.params.documentId, status: "published" }) : null;
    const deletingDraftOnlyDocument = context.action === "delete" && isDraftAndPublish && publishedDocumentToDelete === null;
    const result = await next();
    if (context.contentType.uid !== `plugin::${PLUGIN_ID}.staging-status` && // Prevent infinite loop
    ...
    triggerActions.includes(context.action) && !deletingDraftOnlyDocument) {
      await strapi2.plugin(PLUGIN_ID).service("stagingStatus").set({ unstagedUpdates: true });
    }
    return result;
  });
};
...
const controllers = { githubActions: githubActionsController, config: configController, stagingStatus: stagingStatusController, notifications: notificationsController };
const middlewares = {};
const checkBearerToken = (policyContext, config2, { strapi: strapi2 }) => {
  const notifications = strapi2.plugin(PLUGIN_ID).config("notifications");
...
const configService = ({ strapi: strapi2 }) => ({
  get() {
    try {
      ...
      const workflowID = strapi2.plugin(PLUGIN_ID).config("workflowID");
      ...
      const hideGithubLink = strapi2.plugin(PLUGIN_ID).config("hideGithubLink");
  ...
  return async (config2) => {
    ...
    try {
      ...
      if (err && err.name === "TypeError" && /Load failed|fetch/i.test(err.message)) {
        throw Object.assign(
          new AxiosError("Network Error", AxiosError.ERR_NETWORK, config2, request, err && err.response),
          {
            cause: err.cause || err
          }
        );
...
class Axios {
  ...
  _request(configOrUrl, config2) {
    ...
    this.interceptors.request.forEach(function unshiftRequestInterceptors(interceptor) {
      ...
      const legacyInterceptorReqResOrdering = transitional3.legacyInterceptorReqResOrdering;
...
const githubActionsService = ({ strapi: strapi2 }) => ({
  ...
  async history() {
    try {
      ...
      const environmentHistory = await Promise.all(
        history.map(async (workflow) => {
          try {
            ...
            return jobs.data.jobs.find(
              (job) => job.name.includes(environment)
            ) !== void 0;
          } catch {
            return false;
          }
...
const stagingStatusService = ({ strapi: strapi2 }) => ({
  async get() {
    try {
      const unstagedUpdates = await strapi2.documents(`plugin::${PLUGIN_ID}.staging-status`).findMany({
        sort: "createdAt:desc"
        // Most recent first
      });
      ...
      if (unstagedUpdates.length > 1) {
        unstagedUpdates.forEach(async (doc, idx) => {
          if (idx !== 0) {
            await strapi2.documents(`plugin::${PLUGIN_ID}.staging-status`).delete({ documentId: doc.documentId });
          }
        });
      }
      return currDocument;
...
const emailAutogenerated = "<small>Questa email è stata generata automaticamente</small>";
...
const notificationsService = ({ strapi: strapi2 }) => ({
  async send(event) {
    try {
      ...
      const staging = strapi2.plugin(PLUGIN_ID).config("staging");
      const notifications = strapi2.plugin(PLUGIN_ID).config("notifications");
      const url = strapi2.config.get("server.url");
      if (!notifications || !notifications.enabled) {
      ...
      if (emails.length > 0) {
        await strapi2.plugin("email").services.email.send({
          ...
          html: notificationBody(
            event === "staging-trigger" || event === "staging-end" ? staging?.workflowID ?? "" : workflowID,
            url
          )[event]
        });
      }
  ...
  async deleteEmail(emailToDelete) {
    try {
      const docToDelete = await strapi2.documents(`plugin::${PLUGIN_ID}.email-for-notifications`).findFirst({
        filters: {
          email: {
            $eqi: emailToDelete
          }
...
const services = { githubActions: githubActionsService, config: configService, stagingStatus: stagingStatusService, notifications: notificationsService };
const index = {
  register,
  bootstrap,
  destroy,
